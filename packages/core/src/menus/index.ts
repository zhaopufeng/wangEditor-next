...
class Menus {
    ...
    */
    public init(editor: Editor): void {
        // 获取所有注册的菜单数据
        const allMenuConstructorList: MenuListType = Object.assign({}, Editor.globalCustomMenuConstructorList, editor.customMenuConstructorList)
    ...
    */
    private getCreateMenuNameList(editor: Editor, allMenuConstructorList: MenuListType): string[] {
        // 获取配置项
        const { config: { menus, excludeMenus } } = editor

        // 获取配置菜单 - 当配置菜单为空的时候默认拿所有注册过的菜单名称
        const configMenus = (Array.isArray(menus) && menus.length)
