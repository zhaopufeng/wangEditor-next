...
export default function createEditorElement(editor: Editor) {
  ...
  // 设置编辑区域的属性和样式
  $text.attr('contenteditable', 'true').css('width', '100%').css('height', '100%')

  // placeholder
  const $placeholder = $(`<div>${editor.config.placeholder}</div>`).addClass('placeholder')
  ...
  // 编辑区域样式和属性设置
  $text.addClass('w-e-text').attr('id', getRandom('text-elem'))

  // 编辑区域容器子节点和样式设置
  $textContainer.append($text).append($placeholder).addClass('w-e-text-container')

  // 工具栏样式和属性设置
  $toolbar.addClass('w-e-toolbar').attr('id', getRandom('toolbar-elem'))
